...
export default function CheckoutPage() {
  ...
  function handleSubmit(e) {
    ...
    if (!form.fullName || !form.phone || !form.email || !form.address || !form.city || !form.state || !form.pincode) {
      dispatch(setNotification({ message: '⚠️ Please fill out all fields.', type: 'error' }));
      return;
    }

    // Process order
    dispatch(setNotification({ message: '🎉 Order placed successfully! Thank you for ordering from The Bunny Treats! 🐰🍰', type: 'success' }));
    dispatch(clearCart());
    navigate('/');
  }
      ...
      <div className="empty-cart-container" style={{ position: 'relative' }}>
        {import.meta.env.DEV && (
          <small
            ...
            title="CheckoutPage renders (Redux Toolkit)"
          >
            #{renderCount.current}
          </small>
        )}
